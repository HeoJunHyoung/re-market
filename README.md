...
기존 중고 거래 플랫폼에서 빈번하게 발생하는 **허위 매물, 사기, 신뢰도 저하 문제**를 해결하기 위해 개발된 위치 기반 커머스 서비스입니다.

**Re-Market**은 단순한 거래 중개를 넘어, **엄격한 위치 인증**과 데이터 기반의 **안심 지수(Safety Score)** 시스템을 도입하여 '믿을 수 있는 이웃' 간의 연결을 기술적으로 보장합니다. 

또한, 대규모 트래픽 상황에서도 안정적인 **선착순 나눔**과 **실시간 채팅** 경험을 제공하기 위해 동시성 제어 및 최적화 기술이 적용되었습니다.

---

## 🎯 앱 주요 기능

### 1. 확실한 이웃 인증 (Location Auth)
<table>
  <tr>
...
GPS를 이용하여 사용자의 실제 위치가 설정된 동네(행정동 반경 3km) 내에 있는지 검증합니다. 인증된 '진짜 이웃'끼리만 거래할 수 있게 함으로써 사기 위험의 가능성을 최소화 하였습니다.
> **[Check Point]** 허위 위치 등록 방지 로직 적용
